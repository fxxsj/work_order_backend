...
ENABLE_PERFORMANCE_MONITORING = (
    os.environ.get("ENABLE_PERFORMANCE_MONITORING", "False") == "True"
)
...
# 开发环境从 static/ 源目录提供静态文件
# 生产环境通过 Web 服务器提供 collectstatic 收集到 staticfiles/ 的文件
...
SPECTACULAR_SETTINGS = {
    ...
    ## 主要功能
    - **施工单管理**: 创建、编辑、审核、同步与导出
    - **任务管理**: 任务分派、认领、进度更新、批量操作
    - **工序管理**: 工序开始/完成、批量创建与分派
    - **基础数据**: 客户、部门、工序、用户权限
    - **资产与物料**: 图稿/刀模/版管理、物料与采购流程
    - **产品与库存**: 产品与套装、入库/出库/发货/质检
    - **销售与财务**: 客户订单、发票、收款、对账与统计
    - **审核与通知**: 多级审核、系统通知与告警
    - **监控统计**: 业务指标与系统健康检查
