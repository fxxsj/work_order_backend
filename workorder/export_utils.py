...
def export_products(queryset, filename=None):
    ...
    # 表头
    headers = ['编码', '名称', '规格', '单位', '单价', '库存数量', '最小库存', '产品类型', '产品组', '描述', '是否启用', '创建时间']
    ...
    for row_num, product in enumerate(queryset, 2):
        ...
        ws.cell(row=row_num, column=5, value=str(product.unit_price) if product.unit_price else '0')
        ...
        ws.cell(row=row_num, column=8, value=product_type_map.get(product.product_type, product.product_type or ''))
        ws.cell(row=row_num, column=9, value=product.product_group.name if product.product_group else '')
        ws.cell(row=row_num, column=10, value=product.description or '')
        ws.cell(row=row_num, column=11, value='是' if product.is_active else '否')
        ...
        for row_num, row in enumerate(row_iterator, 2):
            try:
                ...
                if type_idx and type_idx <= len(row):
                    ...
                    data['product_type'] = product_type_map.get(type_str, 'single')
                ...
                if active_idx and active_idx <= len(row):
                    ...
                    data['is_active'] = str(val).strip() in ('是', 'yes', 'true', '1', '启用')
                ...
                if code_lower in imported_codes:
                    errors.append(f'第{row_num}行: 产品编码 "{data["code"]}" 在本次导入中已出现，将跳过')
                    error_count += 1
                    continue
                imported_codes.add(code_lower)

                # 按编码查找是否已存在
                existing = Product.objects.filter(code__iexact=data['code']).first()

                if existing:
                    # 更新现有产品
                    for field in ['name', 'specification', 'unit', 'unit_price',
                                  'stock_quantity', 'min_stock_quantity', 'product_type',
