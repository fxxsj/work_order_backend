...
import pytest
from workorder.models.core import WorkOrderTask, WorkOrderProcess
...
class TestWorkOrderModel:
    ...
    def test_order_number_uniqueness(self):
        ...
        work_order1 = TestDataFactory.create_workorder(
            username=f"user1_{uuid.uuid4().hex[:8]}"
        )
        work_order2 = TestDataFactory.create_workorder(
    ...
    def test_progress_with_completed_processes(self):
        ...
        _ = TestDataFactory.create_workorder_process(
            work_order=work_order, status="completed"
        )
...
class TestWorkOrderTaskModel:
    ...
    def test_task_assignment(self):
        ...
        work_order = TestDataFactory.create_workorder(
            username=f"wo_{uuid.uuid4().hex[:8]}"
        )
        process = TestDataFactory.create_workorder_process(
            work_order=work_order
        )
        department = TestDataFactory.create_department(
            name=f"印刷部_{uuid.uuid4().hex[:4]}"
        )
...
class TestWorkOrderProcessModel:
    ...
    def test_default_status(self):
        ...
        wo_process = WorkOrderProcess.objects.create(
            work_order=work_order, process=process, sequence=1
        )
    ...
    def test_can_start_sequential_process(self):
        ...
        _ = WorkOrderProcess.objects.create(
            work_order=work_order,
            process=process1,
            sequence=1,
            status="completed",
        ...
        assert wo_process2.can_start() is True
    ...
    def test_can_start_parallel_process(self):
        ...
        assert wo_process.can_start() is True
