...
class ProcessModelTest(TestCase):
    ...
    def test_get_required_plates(self):
        ...
        process = Process.objects.create(
            name="印刷", code="PRINT", requires_artwork=True, requires_die=True
        )
...
class ProcessSerializerTest(TestCase):
    ...
    def test_validate_builtin_code_change(self):
        """测试内置工序的编码不可修改"""
        process = Process.objects.create(
            name="印刷", code="PRINT", is_builtin=True
        )

        serializer = ProcessSerializer(
            process, data={"code": "PRINT-NEW", "name": "印刷"}
        )
    ...
    def test_validate_standard_duration_negative(self):
        """测试标准工时不能为负数"""
        serializer = ProcessSerializer(
            data={"code": "TEST", "name": "测试", "standard_duration": -10}
        )
    ...
    def test_validate_standard_duration_too_large(self):
        """测试标准工时不能过大"""
        serializer = ProcessSerializer(
            data={"code": "TEST", "name": "测试", "standard_duration": 10000}
        )
    ...
    def test_validate_sort_order_negative(self):
        """测试排序值不能为负数"""
        serializer = ProcessSerializer(
            data={"code": "TEST", "name": "测试", "sort_order": -1}
        )
...
class ProcessAPITest(APITestCase):
    ...
    def setUp(self):
        ...
        self.process = Process.objects.create(
            name="印刷", code="PRINT", sort_order=1
        )
    ...
    def test_create_process(self):
        """测试创建工序"""
        url = reverse("process-list")
        data = {"code": "DIE-CUT", "name": "模切", "sort_order": 2}
    ...
    def test_update_process(self):
        ...
        data = {"code": "PRINT", "name": "印刷工序", "sort_order": 1}
    ...
    def test_delete_builtin_process(self):
        ...
        builtin_process = Process.objects.create(
            name="内置工序", code="BUILTIN", is_builtin=True
        )
    ...
    def test_batch_update_active(self):
        ...
        process2 = Process.objects.create(
            name="烫金", code="FOIL", is_active=True
        )

        url = reverse("process-batch-update-active")
        data = {"ids": [self.process.id, process2.id], "is_active": False}
        response = self.client.post(url, data, format="json")
    ...
    def test_batch_disable_builtin_process(self):
        """测试不能批量禁用内置工序"""
        builtin_process = Process.objects.create(
            name="内置工序", code="BUILTIN", is_builtin=True, is_active=True
        )
    ...
    def test_filter_by_is_active(self):
        ...
        Process.objects.create(
            name="禁用工序", code="DISABLED", is_active=False
        )
