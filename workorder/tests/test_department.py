...
class DepartmentModelTest(TestCase):
    ...
    def setUp(self):
        ...
        self.root_dept = Department.objects.create(
            name="测试总部", code=self.root_code, sort_order=0, is_active=True
        )
    ...
    def test_get_full_name(self):
        ...
        self.assertEqual(
            self.child_dept.get_full_name(), "测试总部 - 测试生产部"
        )
...
class DepartmentAPITest(APITestCase):
    ...
    def setUp(self):
        ...
        self.active_base_count = Department.objects.filter(
            is_active=True
        ).count()
    ...
    def test_tree_action(self):
        ...
        root = next(
            (item for item in data if item["id"] == self.root_dept.id), None
        )
        self.assertIsNotNone(root)
        self.assertTrue(
            any(
                child["id"] == self.child_dept.id
    ...
    def test_all_action_filter_active(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            len(response.data["data"]), self.active_base_count + 1
        )
