...
class ProductSerializer(serializers.ModelSerializer):
    ...
    product_group_name = serializers.CharField(
        source="product_group.name", read_only=True
    )
    product_group_code = serializers.CharField(
        source="product_group.code", read_only=True
    )
...
class ProductGroupSerializer(serializers.ModelSerializer):
    ...
    items_write = ProductGroupItemWriteSerializer(
        many=True, write_only=True, required=False, source="items"
    )
        ...
        with transaction.atomic():
            ...
            for item_data in items_data:
                ...
                ProductGroupItem.objects.create(
                    product_group=product_group, **item_data
                )
