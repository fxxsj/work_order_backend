...
class ApprovalEscalationSerializer(serializers.ModelSerializer):
    ...
    escalated_by_name = serializers.CharField(
        source="escalated_by.username", read_only=True
    )
...
class WorkflowDeterminationSerializer(serializers.Serializer):
    ...
    def to_representation(self, instance):
        ...
        return {
            ...
            "workflow_type_display": dict(ApprovalWorkflow.WORKFLOW_TYPES).get(
                workflow_type
            ),
            "total_amount": work_order.total_amount,
            "priority": work_order.priority,
            "determination_reason": self._get_determination_reason(
                work_order, workflow_type
            ),
        }
...
class ApprovalStatusSerializer(serializers.Serializer):
    ...
    def to_representation(self, instance):
        ...
        # 获取审核步骤
        steps = ApprovalStep.objects.filter(work_order=work_order).order_by(
            "step_order"
        )
        ...
        # 获取最新上报
        latest_escalation = (
            ApprovalEscalation.objects.filter(work_order=work_order)
            .order_by("-created_at")
            .first()
        )
        ...
        return {
            ...
            "current_step": (
                ApprovalStepSerializer(current_step).data if current_step else None
            ),
            "all_steps": ApprovalStepSerializer(steps, many=True).data,
            "total_steps": steps.count(),
            "completed_steps": steps.filter(status="completed").count(),
            "progress_percentage": (
                (steps.filter(status="completed").count() / steps.count() * 100)
                if steps.count() > 0
                else 0
            ),
            "latest_escalation": (
                ApprovalEscalationSerializer(latest_escalation).data
                if latest_escalation
                else None
            ),
            "urgency_level": (
                UrgentOrderService.calculate_urgency_level(work_order)
                if work_order.priority == "urgent"
                else 0
            ),
