...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--analyze", action="store_true", help="分析现有查询性能"
        )
    ...
    def optimize_indexes(self, dry_run=False):
        ...
        if dry_run:
            ...
            for index_info in indexes_to_create:
                self.stdout.write(
                    f"表: {index_info['table']}, 索引: {index_info['index']}"
                )
            return
        ...
        with transaction.atomic():
            ...
            for index_info in indexes_to_create:
                try:
                    ...
                    created_count += 1
                    self.stdout.write(
                        f"✓ 创建索引: {index_info['table']}.{index_info['index']}"
                    )
                except Exception as e:
                    self.stdout.write(
                        f"✗ 创建索引失败: "
                        f"{index_info['table']}.{index_info['index']} - "
                        f"{str(e)}"
                    )
    ...
    def analyze_table_sizes(self):
        ...
        if "postgresql" in connection.settings_dict["ENGINE"]:
            with connection.cursor() as cursor:
                ...
                for table in tables:
                    self.stdout.write(
                        f"{table[0]}.{table[1]}: {table[2]} ({table[3]} bytes)"
                    )
