...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--fix", action="store_true", help="自动修复发现的问题"
        )
    ...
    def handle(self, *args, **options):
        ...
        # 执行检查
        check_results = DataConsistencyManager.run_consistency_check(
            check_type
        )
        ...
        if auto_fix:
            ...
            fix_results = DataConsistencyManager.auto_fix_consistency_issues(
                fix_type
            )
    ...
    def _display_check_results(self, results):
        ...
        for check_type, data in results["results"].items():
            ...
            if check_type == "stock":
                ...
                self.stdout.write(
                    f"不一致产品数: {data['inconsistent_count']}"
                )
    ...
    def _display_fix_results(self, results):
        ...
        for fix_type, data in results["results"].items():
            ...
            if fix_type == "stock":
                ...
                self.stdout.write(
                    f"需要人工审核: {'是' if data['requires_manual_review'] else '否'}"
                )
