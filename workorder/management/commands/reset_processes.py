...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if work_order_processes_count > 0 and not force:
            ...
            self.stdout.write(
                self.style.WARNING("如果继续，这些施工单工序记录将被删除。")
            )
            self.stdout.write(
                self.style.WARNING(
        ...
        # 检查产品默认工序关联
        products_with_processes = Product.objects.filter(
            default_processes__isnull=False
        ).distinct()
        ...
        try:
            with transaction.atomic():
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"✓ 已清除 {products_count} 个产品的默认工序关联"
                    )
                )
                ...
                if force and work_order_processes_count > 0:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"✓ 已删除 {work_order_processes_count} 个施工单工序记录"
                        )
                    )
                ...
                self.stdout.write(self.style.SUCCESS("工序数据重置完成！"))
                self.stdout.write("")
                self.stdout.write(
                    "预设的21个工序已加载，code字段在Admin中为只读。"
                )
                self.stdout.write("")

        except Exception as e:
            self.stdout.write(self.style.ERROR(f"✗ 重置失败: {e}"))
            raise
