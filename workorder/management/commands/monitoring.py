...
class Command(BaseCommand):
    ...
    def collect_metrics(self, options):
        ...
        if options.get("output"):
            ...
            with open(options["output"], "w", encoding="utf-8") as f:
                json.dump(
                    metrics, f, ensure_ascii=False, indent=2, default=str
                )
            self.stdout.write(
                self.style.SUCCESS(f'指标已输出到: {options["output"]}')
            )
        else:
            # 输出到控制台
            self.stdout.write(
    ...
    def health_check(self, options):
        ...
        for check_name, check_result in health["checks"].items():
            if check_result["status"] == "ok":
                ...
                self.stdout.write(
                    f"  ✗ {check_name}: {check_result.get('error', '异常')}"
                )
    ...
    def performance_check(self, options):
        ...
        if warnings:
            self.stdout.write(
                self.style.WARNING(f"性能警告: {', '.join(warnings)}")
            )
        else:
        ...
        if stats["slowest_endpoints"]:
            ...
            for i, endpoint in enumerate(stats["slowest_endpoints"][:5], 1):
                self.stdout.write(
                    f"  {i}. {endpoint['name']}: "
                    f"{endpoint['execution_time']:.3f}s"
                )
    ...
    def business_metrics(self, options):
        ...
        self.stdout.write(
            f"  施工单总数: {workorder_metrics['order_stats']['total']}"
        )
        self.stdout.write(
            f"  完成率: "
            f"{workorder_metrics['order_stats']['completion_rate']:.1f}%"
        )
        self.stdout.write(
            f"  审核通过率: "
            f"{workorder_metrics['approval_stats']['approval_rate']:.1f}%"
        )
        self.stdout.write(
            f"  平均完成天数: "
            f"{workorder_metrics['time_metrics']['avg_completion_days']:.1f}天"
        )
        ...
        self.stdout.write(
            f"  任务完成率: {task_metrics['task_stats']['completion_rate']:.1f}%"
        )
        self.stdout.write(
            f"  超时任务数: {task_metrics['time_metrics']['overdue_tasks']}"
        )
        self.stdout.write(
            f"  超时率: {task_metrics['time_metrics']['overdue_rate']:.1f}%"
        )
        ...
        self.stdout.write(
            f"  CPU使用率: "
            f"{system_metrics['system_resources']['cpu_percent']:.1f}%"
        )
        self.stdout.write(
            f"  内存使用率: "
            f"{system_metrics['system_resources']['memory_percent']:.1f}%"
        )
        self.stdout.write(
            f"  磁盘使用率: "
            f"{system_metrics['system_resources']['disk_percent']:.1f}%"
        )
        ...
        if warnings:
            self.stdout.write(
                self.style.WARNING(f"业务警告: {', '.join(warnings)}")
            )
        else:
