...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--enable", action="store_true", dest="enable", help="启用审计日志"
        )
        ...
        parser.add_argument(
            "--days", type=int, dest="days", help="设置日志保留天数"
        )
        parser.add_argument(
            "--process-exports",
            action="store_true",
    ...
    def cleanup_logs(self):
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"✓ 已清理 {deleted_count} 条过期日志（保留期: {retention_days}天）"
            )
        )
    ...
    def toggle_audit_log(self, enable):
        ...
        self.stdout.write(self.style.SUCCESS(f"✓ 审计日志已{status}"))
