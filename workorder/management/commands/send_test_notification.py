...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--user-id", type=int, help="指定接收通知的用户ID")
    ...
    def test_task_notification(self, user_id=None):
        ...
        try:
            created_task = DataConsistencyService.create_task_safely(
                workorder, task_data, created_by=user
            )

            if created_task:
                notification_service.notify_task_assignment(
                    task=created_task, assigned_user=user, assigned_by=user
                )
                self.stdout.write(
                    self.style.SUCCESS(
                        f"✓ 任务分配通知已发送给 {user.username}"
                    )
                )
            else:
    ...
    def test_workorder_notification(self, user_id=None):
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"✓ 施工单审核通知已发送给 {workorder.created_by.username}"
            )
        )
        ...
        if not workorder:
            ...
            if creator:
                workorder = WorkOrder.objects.create(
                    order_number=(
                        f'TEST-{timezone.now().strftime("%Y%m%d%H%M%S")}'
                    ),
                    customer=None,
                    total_amount=1000,
                    priority="normal",
                ...
                self.stdout.write(
                    self.style.ERROR("没有找到用户创建测试施工单")
                )
                return

        notification_service.notify_deadline_warning(workorder, 1)
        self.stdout.write(self.style.SUCCESS("✓ 交货期预警通知已发送"))
