...
# 注意：Mixin 的顺序很重要！
# 从最底层到最高层：
# 1. BaseWorkOrderTaskViewSet - 基础 CRUD（已包含 TaskExportMixin）
# 2. TaskActionsMixin - 单个任务操作
# 3. TaskBulkMixin - 批量操作
# 4. TaskStatsMixin - 统计和导出
class WorkOrderTaskViewSet(
    TaskStatsMixin, TaskBulkMixin, TaskActionsMixin, BaseWorkOrderTaskViewSet
):
    """
    完整的施工单任务视图集

    通过组合多个 Mixin 实现，每个 Mixin 提供特定的功能：
    - BaseWorkOrderTaskViewSet: 基础的 CRUD 操作和权限控制（包含 TaskExportMixin）
