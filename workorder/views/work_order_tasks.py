...
class WorkOrderTaskViewSet(BaseViewSet):
        ...
        if task.quantity_completed is not None and task.production_quantity:
            if task.quantity_completed > task.production_quantity:
                ...
                raise ValidationError(
                    f"完成数量（{task.quantity_completed}）不能超过生产数量（{task.production_quantity}）"
                )
    ...
    def update_quantity(self, request, pk=None):
        ...
        quantity_defective = request.data.get(
    ...
    def batch_update_quantity(self, request):
        ...
        for task, increment, defective in zip(tasks, increments, defectives):
            try:
                ...
                if new_quantity_completed < 0:
                    failed_tasks.append(
                        {"task_id": task.id, "error": "更新后完成数量不能小于0"}
                    )
                    continue
                ...
                TaskLog.objects.create(
                    task=task,
                    log_type="update_quantity",
                    content=f"批量更新完成数量：{quantity_before} → {new_quantity_completed}，本次完成：{increment}，不良品：{defective}"
                    + (f"，备注：{notes}" if notes else ""),
                    quantity_before=quantity_before,
                    quantity_after=new_quantity_completed,
    ...
    def batch_complete(self, request):
        ...
        for task in tasks:
            try:
                # 检查任务状态
                if task.status == "completed":
                    failed_tasks.append({"task_id": task.id, "error": "任务已经完成"})
                    continue

                if task.status == "cancelled":
                    failed_tasks.append(
                        {"task_id": task.id, "error": "已取消的任务无法完成"}
                    )
                    continue
    ...
    def batch_cancel(self, request):
        ...
        for task in tasks:
            try:
                # 检查任务状态
                if task.status == "cancelled":
                    failed_tasks.append({"task_id": task.id, "error": "任务已经取消"})
                    continue

                if task.status == "completed":
                    failed_tasks.append(
                        {"task_id": task.id, "error": "已完成的任务无法取消"}
                    )
                    continue
    ...
    def batch_assign(self, request):
        ...
        from ..models.core import Department, TaskLog
    ...
    def collaboration_stats(self, request):
        ...
        for operator in operators:
            ...
            total_completed_quantity = (
                completed_task_data["total_completed_quantity"] or 0
            )
            total_defective_quantity = (
                completed_task_data["total_defective_quantity"] or 0
            )
            total_production_quantity = (
                completed_task_data["total_production_quantity"] or 0
            )

            # 计算不良品率
            defective_rate = 0
            if total_completed_quantity > 0:
                defective_rate = round(
                    (total_defective_quantity / total_completed_quantity) * 100, 2
                )

            # 统计平均完成时间（从任务创建到完成的时间）
            avg_completion_hours = None
            completed_tasks_with_times = tasks.filter(
                status="completed", created_at__isnull=False
            ).filter(time_filter)
            ...
            # 获取操作员所属部门
            departments = (
                operator.profile.departments.all()
        ...
        return Response(
            {
                ...
                "summary": {
                    ...
                    "total_completed_tasks": sum(
                        s["completed_tasks"] for s in stats_list
                    ),
                    "total_completed_quantity": sum(
                        s["total_completed_quantity"] for s in stats_list
                    ),
                    "total_defective_quantity": sum(
                        s["total_defective_quantity"] for s in stats_list
                    ),
                    "overall_defective_rate": round(
                        (
                            (
                                sum(s["total_defective_quantity"] for s in stats_list)
                                / sum(s["total_completed_quantity"] for s in stats_list)
                                * 100
                            )
                            if sum(s["total_completed_quantity"] for s in stats_list)
                            > 0
                            else 0
                        ),
                        2,
    ...
    def bulk_update(self, request):
        ...
        for task in tasks:
            if (
                "production_quantity" in validated_data
                and validated_data["production_quantity"] is not None
            ):
            ...
            if (
                "production_requirements" in validated_data
                and validated_data["production_requirements"] is not None
            ):
        ...
        if update_fields and tasks:
            ...
            WorkOrderTask.objects.bulk_update(tasks, unique_fields, batch_size=100)
    ...
    def bulk_delete(self, request):
        ...
        if not task_ids:
            return Response(
                {"error": "请提供要删除的任务ID"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if tasks.count() != len(task_ids):
            return Response(
                {"error": "只能删除草稿状态的任务"}, status=status.HTTP_400_BAD_REQUEST
            )
