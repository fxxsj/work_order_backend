...
class Department(models.Model):
    ...
    name = models.CharField(
        "部门名称", max_length=50, unique=True, help_text="部门名称，必须唯一"
    )
    ...
    sort_order = models.IntegerField(
        "排序", default=0, help_text="排序值，数字越小越靠前"
    )
...
class Process(models.Model):
    ...
    # 工序需要的版（配置化）
    requires_artwork = models.BooleanField(
        "需要图稿", default=False, help_text="该工序是否需要图稿（CTP版）"
    )
    requires_die = models.BooleanField(
        "需要刀模", default=False, help_text="该工序是否需要刀模"
    )
    requires_foiling_plate = models.BooleanField(
        "需要烫金版", default=False, help_text="该工序是否需要烫金版"
    )
    requires_embossing_plate = models.BooleanField(
        "需要压凸版", default=False, help_text="该工序是否需要压凸版"
    )
