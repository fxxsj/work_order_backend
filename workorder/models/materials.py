...
class MaterialSupplier(models.Model):
    ...
    lead_time_days = models.IntegerField(
        "采购周期（天）", default=7, help_text="该供应商的采购周期"
    )
    notes = models.TextField("备注", blank=True)
    created_at = models.DateTimeField("创建时间", auto_now_add=True)
...
class PurchaseOrder(models.Model):
    ...
    rejection_reason = models.TextField(
        "拒绝原因", blank=True, help_text="如果采购单被拒绝，填写拒绝原因"
    )
    created_at = models.DateTimeField("创建时间", auto_now_add=True)
    updated_at = models.DateTimeField("更新时间", auto_now=True)
...
class PurchaseOrderItem(models.Model):
    ...
    material = models.ForeignKey(
        Material, on_delete=models.PROTECT, verbose_name="物料"
    )
    quantity = models.DecimalField("采购数量", max_digits=10, decimal_places=2)
    received_quantity = models.DecimalField(
        "已收货数量", max_digits=10, decimal_places=2, default=0
    )
    ...
    def qualified_quantity(self):
        ...
        return sum(
            ...
            for record in self.receive_records.filter(
                inspection_status="partial_qualified"
            )
        )
...
class PurchaseReceiveRecord(models.Model):
    ...
    # 收货信息
    received_quantity = models.DecimalField(
        "收货数量", max_digits=10, decimal_places=2, help_text="本次收货的数量"
    )
    received_date = models.DateField(
        "收货日期", default=timezone.now, help_text="实际收货日期"
    )
    ...
    delivery_note_number = models.CharField(
        "送货单号", max_length=100, blank=True, help_text="供应商送货单号"
    )
    ...
    unqualified_reason = models.TextField(
        "不合格原因", blank=True, help_text="质检不合格的原因说明"
    )
    ...
    inspected_at = models.DateTimeField("质检时间", null=True, blank=True)

    # 入库信息
    is_stocked = models.BooleanField(
        "已入库", default=False, help_text="合格物料是否已入库"
    )
    stocked_at = models.DateTimeField("入库时间", null=True, blank=True)
    ...
    is_returned = models.BooleanField(
        "已退货", default=False, help_text="不合格物料是否已退货"
    )
    returned_quantity = models.DecimalField(
        "退货数量", max_digits=10, decimal_places=2, null=True, blank=True
    )
    ...
    return_note = models.TextField("退货备注", blank=True)
