...
class WorkOrder(models.Model):
    ...
    product_group_item = models.ForeignKey(
        "workorder.ProductGroupItem",
    ...
    printing_cmyk_colors = models.JSONField(
        "印刷CMYK颜色",
        default=list,
        blank=True,
        help_text='选中的CMYK颜色，如：["C", "M", "K"]',
    )
    printing_other_colors = models.JSONField(
        "印刷其他颜色",
        default=list,
        blank=True,
    ...
    defective_quantity = models.IntegerField(
        "预损数量", null=True, blank=True, help_text="单位：车"
    )
...
class WorkOrderProcess(models.Model):
    ...
    process = models.ForeignKey(
        "workorder.Process", on_delete=models.PROTECT, verbose_name="工序"
    )
    ...
    def can_start(self):
        ...
        non_parallel_processes = (
            WorkOrderProcess.objects.filter(work_order=self.work_order)
            .exclude(process__is_parallel=True)
            .exclude(process__code__in=[ProcessCodes.CTP, ProcessCodes.DIE])
            .order_by("sequence")
        )
    ...
    def check_and_update_status(self):
        ...
        for task in tasks:
            ...
            if (
                ...
                and task.quantity_completed < task.production_quantity
            ):
                return False
        ...
        if self.department:
            ...
            if assigned_dept:
                ...
                if available_departments.exists():
                    task.assigned_department = available_departments.order_by(
                        "sort_order"
                    ).first()
    ...
    def _update_product_stock_on_packaging(self):
        ...
        with transaction.atomic():
            ...
            packaging_tasks = (
                self.tasks.filter(task_type="packaging", status="completed")
            ...
            for task in packaging_tasks:
                ...
                actual_quantity_to_stock = task.quantity_completed - (
            ...
            if product_quantities:
                ...
                for product_id, quantity in product_quantities.items():
                    ...
                    stock_updates.append(
                        (product, old_quantity, new_quantity, quantity)
                    )
...
class WorkOrderMaterial(models.Model):
    ...
    purchase_date = models.DateField(
        "采购日期", null=True, blank=True, help_text="采购下单日期"
    )
    received_date = models.DateField(
        "回料日期", null=True, blank=True, help_text="物料回料日期"
    )
...
class TaskLog(models.Model):
    ...
    quantity_increment = models.IntegerField(
...
class WorkOrderTask(models.Model):
    ...
    production_quantity = models.IntegerField(
        "生产数量", default=0, help_text="该任务需要生产的数量"
    )
    quantity_completed = models.IntegerField(
        "完成数量", default=0, help_text="任务完成数量，可自动计算或手动输入"
    )
    quantity_defective = models.IntegerField(
        "不良品数量", default=0, help_text="任务不良品数量，在任务完成时记录"
    )
    auto_calculate_quantity = models.BooleanField(
        "自动计算数量", default=True, help_text="是否自动计算完成数量"
    )
    ...
    production_requirements = models.TextField(
        "生产要求", blank=True, help_text="生产过程中的特殊要求"
    )
    stock_accounted_quantity = models.IntegerField(
        "已计入库存的完成数量",
        default=0,
