...
class ProductStock(models.Model):
    ...
    product = models.ForeignKey(
        "workorder.Product", on_delete=models.CASCADE, verbose_name="产品"
    )
    quantity = models.DecimalField(
        "库存数量", max_digits=10, decimal_places=2, default=0
    )
    reserved_quantity = models.DecimalField(
        "预留数量", max_digits=10, decimal_places=2, default=0
    )
    ...
    unit_cost = models.DecimalField(
        "单位成本", max_digits=10, decimal_places=2, default=0
    )
    location = models.CharField(
        "库位", max_length=50, blank=True, help_text="如: A01-01-01"
    )
    ...
    status = models.CharField(
        "状态", max_length=20, choices=STATUS_CHOICES, default="in_stock"
    )
...
class StockOut(models.Model):
    ...
    out_type = models.CharField(
        "出库类型", max_length=20, choices=TYPE_CHOICES, default="delivery"
    )
...
class DeliveryOrder(models.Model):
    ...
    package_weight = models.DecimalField(
        "总重量(kg)", max_digits=10, decimal_places=2, null=True, blank=True
    )
...
class QualityInspection(models.Model):
    ...
    result = models.CharField(
        "检验结果", max_length=20, choices=RESULT_CHOICES, default="pending"
    )
    ...
    defective_rate = models.DecimalField(
        "不良率", max_digits=5, decimal_places=2, default=0
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.inspection_quantity > 0:
            self.defective_rate = (
                self.failed_quantity / self.inspection_quantity
            ) * 100
