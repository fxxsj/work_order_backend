...
class AuditLog(models.Model):
    ...
    username = models.CharField(
        max_length=150, blank=True, verbose_name="用户名快照"
    )
    ...
    object_repr = models.CharField(
        max_length=255, blank=True, verbose_name="对象表示"
    )
    ...
    request_method = models.CharField(
        max_length=10, blank=True, verbose_name="请求方法"
    )
    ...
    def __str__(self):
        return (
            f"{self.get_action_type_display()} - {self.object_repr} "
            f"by {self.username}"
        )
...
class AuditLogSettings(models.Model):
    ...
    retention_days = models.IntegerField(
        default=365, help_text="日志保留天数，超过此天数的日志将被自动清理"
    )

    # 是否启用审计日志
    enabled = models.BooleanField(
        default=True, help_text="是否启用审计日志功能"
    )
