...
class WorkOrderValidator:
    ...
    def validate_asset_process_match(self):
        ...
        if (
            ...
            and not self.work_order.foiling_plates.exists()
        ):
            process_names = ", ".join(
                [p.name for p in processes_requiring_foiling_plate]
            )
            self.errors.append(
        ...
        if (
            ...
            and not self.work_order.embossing_plates.exists()
        ):
            process_names = ", ".join(
                [p.name for p in processes_requiring_embossing_plate]
            )
            self.errors.append(
                f"选择了需要压凸版的工序（{process_names}），请至少选择一个压凸版"
            )
    ...
    def validate_process_sequence(self):
        ...
        """
        processes_ordered = (
            self.work_order.order_processes.filter(
