...
class QueryOptimizer:
    ...
    @staticmethod
    def batch_get_workorders_with_stats(
        order_ids: List[int],
        ...
        # 使用子查询获取统计信息，避免多次查询
        work_orders = (
            WorkOrder.objects.filter(id__in=order_ids)
...
class PerformanceOptimizedManager(models.Manager):
    ...
    def get_optimized_queryset(self, include_details=False):
        ...
        if hasattr(self.model, "_get_optimized_queryset"):
            return self.model._get_optimized_queryset(
                super().get_queryset(), include_details
            )
        ...
        return queryset.annotate(
            ...
            progress_percentage=models.Case(
                models.When(
                    ...
                    then=models.F("completed_processes")
                    * 100.0
                    / models.F("total_processes"),
                ),
                default=0,
