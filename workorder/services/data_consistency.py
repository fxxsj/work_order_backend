...
class StockConsistencyService:
    ...
    def validate_stock_consistency(product) -> Dict[str, any]:
        ...
        if difference != 0:
            issues.append(
                f"库存数量不一致：实际{actual_quantity}，"
                f"预期{expected_quantity}，差异{difference}"
            )
    ...
    def fix_stock_consistency(product, user=None) -> Dict[str, any]:
        ...
        validation = StockConsistencyService.validate_stock_consistency(
            product
        )
        ...
        fix_log = ProductStockLog.objects.create(
            product=product,
            change_type=(
                "add" if expected_quantity > old_quantity else "reduce"
            ),
        ...
        logger.info(
            f"产品 {product.code} 库存一致性已修复：{old_quantity} -> {expected_quantity}"
        )
...
class WorkOrderQuantityService:
    ...
    def get_quantity_summary(work_order) -> Dict[str, any]:
        ...
        total_product_quantity = sum(
            item.quantity or 0 for item in product_items
        )
        ...
        return {
            ...
            "quantity_consistency": {
                ...
                "task_vs_process": total_task_quantity
    ...
    def validate_quantity_consistency(work_order) -> Dict[str, any]:
        ...
        if summary["order_quantity"] != summary["total_product_quantity"]:
            issues.append(
                f"施工单数量({summary['order_quantity']})与"
                f"产品数量汇总({summary['total_product_quantity']})不一致"
            )
        ...
        if summary["order_quantity"] != summary["total_task_quantity"]:
            issues.append(
                f"施工单数量({summary['order_quantity']})与"
                f"任务完成数量汇总({summary['total_task_quantity']})不一致"
            )

        # 检查工序数量一致性
        if summary["total_task_quantity"] != summary["total_process_quantity"]:
            issues.append(
                f"任务完成数量({summary['total_task_quantity']})与"
                f"工序完成数量({summary['total_process_quantity']})不一致"
            )
        ...
        if summary["total_defective_quantity"] < 0:
            issues.append(
                f"不良品数量为负数：{summary['total_defective_quantity']}"
            )
...
class MaterialStockService:
    ...
    def calculate_material_usage(work_order) -> Dict[int, Dict[str, any]]:
        ...
        for material_item in work_order.materials.all():
            ...
            # 计算实际使用量（基于开料任务）
            cutting_tasks = (
                work_order.order_processes.filter(process__code="CUT")
                .prefetch_related("tasks")
                .first()
            )
            ...
            if cutting_tasks:
                for task in cutting_tasks.tasks.filter(
                    material=material_item.material, status="completed"
                ):
    ...
    def check_material_availability(work_order) -> Dict[str, any]:
        ...
        Returns:
            Dict: 检查结果
        """
        material_usage = MaterialStockService.calculate_material_usage(
            work_order
        )
        issues = []
        warnings = []
        ...
        for material_id, usage_info in material_usage.items():
            ...
            # 检查开料状态
            if (
                usage_info["need_cutting"]
                and usage_info["purchase_status"] != "cut"
            ):
            ...
            if hasattr(material, "stock_quantity"):
                if material.stock_quantity < usage_info["actual_usage"]:
                    issues.append(
                        ...
                        f"需要{usage_info['actual_usage']}，"
                        f"库存{material.stock_quantity}"
                    )
...
class DataConsistencyManager:
    ...
    def run_consistency_check(check_type: str = "all") -> Dict[str, any]:
        ...
        Returns:
            Dict: 检查结果
        """
        results = {"check_time": timezone.now(), "results": {}}

        if check_type in ["stock", "all"]:
            results["results"][
                "stock"
            ] = DataConsistencyManager._check_stock_consistency()

        if check_type in ["quantity", "all"]:
            results["results"][
                "quantity"
            ] = DataConsistencyManager._check_quantity_consistency()

        if check_type in ["material", "all"]:
            results["results"][
                "material"
            ] = DataConsistencyManager._check_material_consistency()
        ...
        for product in products:
            ...
            if not validation["is_consistent"]:
                inconsistent_products.append(
                    {"product": product, "validation": validation}
                )
    ...
    def _check_quantity_consistency() -> Dict[str, any]:
        ...
        for work_order in work_orders:
            validation = (
                WorkOrderQuantityService.validate_quantity_consistency(
                    work_order
                )
            )
            if not validation["is_consistent"]:
                inconsistent_orders.append(
                    {"work_order": work_order, "validation": validation}
                )
    ...
    def _check_material_consistency() -> Dict[str, any]:
        ...
        for work_order in work_orders:
            availability = MaterialStockService.check_material_availability(
                work_order
            )
    ...
    @staticmethod
    def auto_fix_consistency_issues(
        fix_type: str = "all", user=None
    ) -> Dict[str, any]:
        """
        自动修复一致性问题
        ...
        Returns:
            Dict: 修复结果
        """
        results = {"fix_time": timezone.now(), "results": {}}

        if fix_type in ["stock", "all"]:
            results["results"]["stock"] = (
                DataConsistencyManager._fix_stock_issues(user)
            )

        if fix_type in ["quantity", "all"]:
            results["results"]["quantity"] = (
                DataConsistencyManager._fix_quantity_issues(user)
            )
    ...
    def _fix_stock_issues(user=None) -> Dict[str, any]:
        ...
        for product in products:
            ...
            if not validation["is_consistent"]:
                try:
                    result = StockConsistencyService.fix_stock_consistency(
                        product, user
                    )
                    if result["success"]:
                        fixed_count += 1
