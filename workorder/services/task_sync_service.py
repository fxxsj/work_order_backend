...
class TaskSyncService:
    ...
    def preview_sync(work_order, old_process_ids, new_process_ids):
        ...
        if removed:
            ...
            blocked_task_ids = list(
                removed_tasks.exclude(status="pending").values_list(
                    "id", flat=True
                )
            )
        ...
        for process in WorkOrderProcess.objects.filter(
            ...
            missing_tasks = TaskGenerationService.build_missing_task_objects(
                process
            )
            if missing_tasks:
    ...
    def execute_sync(work_order, old_process_ids, new_process_ids):
        ...
        """
        # 锁定施工单，防止并发修改
        locked_work_order = WorkOrder.objects.select_for_update().get(
            id=work_order.id
        )
        ...
        if removed_ids:
            ...
            if blocked_task_ids:
                return {
                    ...
                    "blocked_task_ids": blocked_task_ids,
                    "message": (
                        f"同步被阻止：{len(blocked_task_ids)} 个任务已开始或已完成，"
                        f"不能自动删除"
                    ),
                }
        ...
        if added_ids:
            ...
            for process in new_processes:
                ...
                task_objects = (
                    TaskGenerationService.build_missing_task_objects(process)
                )
