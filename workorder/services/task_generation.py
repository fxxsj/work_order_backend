...
class TaskGenerationService:
    ...
    def generate_tasks_and_dispatch(work_order):
        ...
        for work_order_process in work_order.order_processes.all():
            # 为每个工序构建任务对象
            tasks = TaskGenerationService.build_missing_task_objects(
                work_order_process
            )
    ...
    def build_missing_task_objects(work_order_process):
        ...
        task_objects = TaskGenerationService.build_task_objects(
            work_order_process
        )
        return [
            task
    ...
    def _task_exists(task):
        ...
        return (
            WorkOrderTask.objects.filter(**filters)
            .exclude(status="draft")
            .exists()
        )
